...
`spectra.filter` with a retention-time criterion is faster on a reader with random access
(every access strategy except `stream`): the first query caches every spectrum's scan times,
