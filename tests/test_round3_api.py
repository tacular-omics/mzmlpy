...
# ---------------------------------------------------------------- private copies across processes
def _gz_run(tmp_path: Path) -> Path:
    import gzip
...
def _run_script(script: str) -> list[str]:
    import subprocess
    import sys
...
def test_a_forked_child_does_not_delete_the_parents_copy(tmp_path: Path) -> None:
    gz = _gz_run(tmp_path)
    script = (
        "import os, sys, warnings; warnings.simplefilter('ignore')\n"
        "from mzmlpy import Mzml\n"
        f"reader = Mzml({str(gz)!r})\n"
        "live = iter(reader.spectra); next(live)\n"
        "copy = reader._file_object.temporary_copy\n"
        "for read_in_child in (False, True):\n"
        "    pid = os.fork()\n"
        "    if pid == 0:\n"
        "        if read_in_child:\n"
        "            reader.spectra[1]\n"
        "        sys.exit(0)  # a normal exit runs finalizers and atexit hooks in the child\n"
        "    _, status = os.waitpid(pid, 0)\n"
        "    assert status == 0, status\n"
        "    assert os.path.exists(copy)\n"
        "print(copy, len(list(reader.spectra)), reader.spectra[5].id, next(live).id)\n"
        "# exits without close(): the parent deletes its copy at exit\n"
    )
    copy, count, fifth, following = _run_script(script)
    assert (count, fifth, following) == (str(len(TIMES)), "scan=5", "scan=1")
    assert not Path(copy).exists()


def test_exit_closes_iterators_and_deletes_the_copy(tmp_path: Path) -> None:
    gz = _gz_run(tmp_path)
    script = (
        "import warnings; warnings.simplefilter('ignore')\n"
        "from mzmlpy import Mzml\n"
        f"reader = Mzml({str(gz)!r}, gzip_mode='extract')\n"
        "live = iter(reader.spectra); next(live)\n"
        "heads = reader._file_object.iter_spectrum_heads(); next(heads)\n"
        "print(reader._file_object.temporary_copy)\n"
    )
    (copy,) = _run_script(script)
    assert not Path(copy).exists()


def test_clear_cache_keeps_the_copies_of_open_readers(tmp_path: Path) -> None:
    import os
    import subprocess
    import sys
    ...
    gz = _gz_run(tmp_path)
    with Mzml(gz) as reader:
        copy = Path(reader._file_object.temporary_copy or "")  # noqa: SLF001
        assert copy.exists()
        ...
        assert [s.rt for s in reader.spectra.filter(rt_range=(0.0, 4.0))] == [0.0, 2.0, 4.0]
