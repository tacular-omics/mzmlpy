...
def test_file_description(filename):
    ...
    # Access by Accession
    msn_spectrum = fd.file_content.get_cv_param("MS:1000580")
    assert msn_spectrum is not None
    assert msn_spectrum.name == "MSn spectrum"

    # Access by Name
    centroid_spectrum = fd.file_content.get_cv_param("centroid spectrum")
    ...
    assert fd.contact[0].get_cv_param("MS:1000586").value == "William Pennington"
    assert fd.contact[0].get_cv_param("contact organization").value == "Higglesworth University"
    assert fd.contact[0].get_cv_param("MS:1000589").value == "wpennington@higglesworth.edu"
